...
void Buffer::append(const char *data,size_t size)             
{
 ...
 void Buffer::appendwithsep(const char *data,size_t size)  
 {
    ...
    else if (sep_==2)   // HTTP：不自动加分隔符（响应由上层拼 Content-Length + \r\n\r\n）
    {
        buf_.append(data,size);
    }
    // 其它的代码请各位自己完善。
...
bool Buffer::pickmessage(std::string &ss)                           
{
    ...
    else if (sep_==1)          // 四字节的报头。
    {
        ...
        if (buf_.size() < (size_t)len+4) return false;     // 如果buf_中的数据量小于报文头部，说明buf_中的报文内容不完整。
        ss=buf_.substr(4,len);                        // 从buf_中获取一个报文。
        buf_.erase(0,len+4);                          // 从buf_中删除刚才已获取的报文。
        return true;
    }
    else if (sep_==2)   // HTTP：用 \r\n\r\n 分隔（先支持 GET/无body 的最小闭环）
    {
        const std::string sep = "\r\n\r\n";
        size_t pos = buf_.find(sep);
        if (pos == std::string::npos) return false;

        // 取出包含 \r\n\r\n 的“头部报文”
        ss = buf_.substr(0, pos + sep.size());
        buf_.erase(0, pos + sep.size());
        return true;
    }
