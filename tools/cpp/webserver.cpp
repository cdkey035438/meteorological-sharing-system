...
// ===== 字段元数据（中文可读 + 单位 + 缩放系数）=====
// 说明：后端仍输出数据库/业务原始值；前端可根据 meta 中的 scale/unit/label 做展示转换。
struct st_fieldmeta
{
    const char* label;
    const char* unit;
    const char* scale;   // 用字符串避免 double 格式差异，如 0.1/1
    const char* type;    // string/number/datetime
    const char* format;  // datetime format，如 yyyymmddhh24miss
};

static const std::map<std::string, st_fieldmeta> g_fieldmeta = {
    // 通用
    {"obtid",     {"站号",     "",    "1",   "string",  ""}},
    {"obtcname",  {"站名",     "",    "1",   "string",  ""}},
    {"obtnname",  {"站名",     "",    "1",   "string",  ""}},
    {"obtname",   {"站名",     "",    "1",   "string",  ""}},
    ...
    {"ddatetime", {"数据时间", "",    "1",   "datetime","yyyymmddhh24miss"}},
...
// 从T_INTERCFG.colmeta读取字段元数据。
// colmeta格式（推荐）：按colstr顺序，用逗号分隔每个字段；每个字段用“|”分隔 label|unit|scale。
// 例：站号||1,站名||1,气温|℃|0.1
// 注意：unit可为空（用空字符串占位），scale建议填写 0.1/1/0.01 等。
static std::string build_meta_xml(const ccmdstr& cols, const std::string& colmeta)
{
    std::string xml;
    xml.append("<meta>\n");

    // Normalize column names to avoid subtle mismatches (case/whitespace).
    ...
    // 解析colmeta：items.size() == cols.size() 时，按顺序覆盖默认元数据。
    ccmdstr items;
    if (colmeta.empty() == false) items.splittocmd(colmeta, ",");

    for (int ii = 0; ii < cols.size(); ++ii)
    {
        // Always use normalized column name for meta keys.
        const std::string col = lower_copy(trim_copy(cols[ii]));

        // 默认：优先使用内置字典（未配置colmeta时仍保证有 meta）。
        auto it = g_fieldmeta.find(col);
        std::string label = col;
        std::string unit  = "";
        std::string scale = "1";
        std::string type  = "string";
        std::string fmt   = "";
        if (it != g_fieldmeta.end())
        {
        ...
        // 若colmeta有效且按顺序配置了该字段的 label|unit|scale，则覆盖默认值。
        if (items.size() == cols.size())
        {
            ccmdstr one;
            one.splittocmd(items[ii], "|");
            if (one.size() >= 1 && one[0].empty() == false) label = one[0];
            if (one.size() >= 2) unit = one[1];
            if (one.size() >= 3 && one[2].empty() == false) scale = one[2];
        }

        // **Hard safety rule** for unified meta-driven rendering:
        // If scale != 1, it must be numeric (otherwise the front-end won't scale).
        // This prevents mistakes like vis/height being marked as string.
        if (type == "string" && scale != "1") type = "number";

        // 统一协议：<field name="t" label="气温" unit="℃" scale="0.1" type="number"/>
        xml.append(sformat("<field name=\"%s\" label=\"%s\" unit=\"%s\" scale=\"%s\" type=\"%s\"",
                           col.c_str(), label.c_str(), unit.c_str(), scale.c_str(), type.c_str()));
        if (fmt.empty() == false)
            xml.append(sformat(" format=\"%s\"", fmt.c_str()));
        xml.append("/>\n");
    }
...
public:
    ...
    void workfunc(int id)       
    {
        ...
        if (conn.connecttodb("liang/101018@snorcl11g_140","Simplified Chinese_China.AL32UTF8")!=0)
        {
            logfile.write("connect database(liang/101018@snorcl11g_140) failed.\n%s\n",conn.message()); return;
        }
    ...
    void bizmain(connection &conn,const string & recvbuf,string & sendbuf)
    {
       ...
       // 3）判断用户是否有访问接口的权限。
        ...
        string selectsql,colstr,bindin,colmeta;
        ...
        stmt.bindout(4,colmeta,2000);     // 字段元数据（中文/单位/缩放）。
        ...
        sendbuf=sendbuf+build_meta_xml(cmdstr,colmeta);
...
bool getvalue(const string &strget,const string &name,string &value)
{
    // http://192.168.96.140:8080/api?username=liang&passwd=101018
    // GET /api?username=liang&passwd=101018 HTTP/1.1
    // Host: 192.168.96.140:8080
